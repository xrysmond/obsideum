...
function wireProviderEvents(provider) {
  ...
  /* Guard — each provider only gets wired once.
   * The provider init useEffect re-runs on every primaryAddr change;
   * without this every re-render stacks duplicate event handlers. */
  ...
  provider.on('disconnect', function () {
     ...
     * background the tab — NOT a reliable signal that the Privy session ended.
     * Null the provider so the next getSigner() re-fetches it, but do NOT
     * touch STATE.connected — Privy's authenticated flag owns that. */
    window.privyProvider = null;
  });
}
  ...
  return function PrivyBridge() {
    ...
    useEffect(function () {
      window._privyBridge = {
        ...
        user:          user,
        /* initOAuth — full-page redirect to OAuth provider.
         * No popup. Works on Brave mobile. Usage: initOAuth({ provider: 'google' }) */
        initOAuth:     p.initOAuth,
        /* Link methods — require an authenticated session */
    ...
    useEffect(function () {
      ...
      primaryWallet.getEthereumProvider()
        .then(function (provider) {
          /* For external (injected) wallets — Brave, MetaMask — Privy returns
           * a wrapped proxy. On Android the wrapper suppresses eth_sendTransaction
           * before it reaches Brave's native confirmation UI. Use window.ethereum
           * directly so the bottom sheet actually fires.
           * Embedded Privy wallets sign server-side — leave those untouched. */
          var isExternal  = primaryWallet.walletClientType !== 'privy';
          var useProvider = (isExternal && window.ethereum) ? window.ethereum : provider;

          /* Set before alive check — never lose the provider on re-render */
...
async function disconnect() {
  closeWalletSheet();
  if (!window._privyBridge) {
    /* Bridge not ready — clear local state, no redirect */
    window.privyProvider = null;
    setState({ wallet: null, connected: false, ens: null, ensSubname: null, network: null });
    return;
  }
  try {
    ...
    /* Privy's authenticated useEffect fires → clears STATE. Nothing else needed. */
  } catch (err) {
...
/* Wallets array changed — re-render list immediately.
 * state:wallet fires first but reads stale STATE.wallets;
 * this catches the wallets-sync useEffect that fires a frame later. */
...
/* ── Stale session detection ─────────────────────────────────────
   Re-validates Privy auth when the tab regains focus.
   Catches STATE.connected=true after session expired in background. */
...
/* ── Social login helpers ────────────────────────────────────────
   initOAuth triggers a full-page redirect — no popup, works on Brave mobile.
   Exposed globally so any UI element can call them. */

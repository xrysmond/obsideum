...
https://xrysmond.github.io/obsideum/app
...
Same thing with path encoding for multi-hop swaps. The Solidity version is referenced but there's no JavaScript equivalent shown. And the difference between Quoter v1 and QuoterV2 is never explained clearly anywhere. Both need callStatic, but v1 takes positional arguments and QuoterV2 takes a single struct, so the actual call looks completely different in code. There's nothing in the docs that tells you this. You find out when it breaks.
